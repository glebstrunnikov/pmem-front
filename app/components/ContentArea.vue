...
const { data, error, pending } = await useFetch<StrapiResponse>(
  () =>
    `/posts?locale=${locale.value}${query.value
      ? `&filters[$or][0][title][$containsi]=${query.value}&filters[$or][1][content][$containsi]=${query.value}`
      : ""
    }${tagSlug.value ? `&filters[tags][slug][$eq]=${tagSlug.value}` : ""}${sort.value ? `&sort=${sort.value}` : ""
...
<template>
  ...
  {{ locale }} -->
    ...
    <div v-if="(query || tagSlug) && !data?.data.length" class="w-full flex flex-col items-center text-center mt-18">
    ...
    <div v-if="data" :class="contentClass">
      <CardArticleCard v-for="post in allPosts" :key="post.id" :title="post.title" :description="post.lead"
        :imageUrl="config.public.url + post.cover?.url" :date="post.publishedAt" :link="'/posts/' + post.slug"
        :tags="post.tags" />
    </div>
