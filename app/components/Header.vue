...
<template>
  <div class="flex flex-col w-full bg-background z-30 sticky top-0">
    <div class="w-full flex flex-col justify-between bg-background z-30">
      <div class="flex justify-between w-full px-6 sm:px-12 lg:px-18 transition-all duration-300 pt-5 pb-9 items-center"
        :class="scrolled ? '' : 'lg:pt-20 lg:pb-29'">
        <div class="flex items-center flex-shrink-0">
          <router-link to="/">
            <img class="h-15 w-15" :src="logo" alt="" />
          </router-link>
          <div class="font-title text-title-l text-primary hidden lg:block whitespace-nowrap mx-6">
            <router-link to="/">{{ i18n.t("orgName") }}</router-link>
          </div>
        </div>
        <div class="flex-grow"></div>
        <div class="lg:hidden flex justify-end flex-shrink" :class="showMobileMenu ? 'mb-6' : ''">
          <UiBaseIcon :icon="showMobileMenu ? 'cross' : 'burger'" class="cursor-pointer"
            :class="showMobileMenu ? 'max-lg:hidden' : ''" @click="showMobileMenu = !showMobileMenu" />
        </div>
        <div
          ...
          :class="showMobileMenu
            ...
            ">
          <UiBaseIcon icon="cross" class="cursor-pointer" :class="showMobileMenu ? 'lg:hidden' : 'hidden'"
            @click="showMobileMenu = !showMobileMenu" />
          <UiBaseButton :label="i18n.t('news')" :type="isActive('posts') || isMain ? 'transparent-active' : 'transparent'
            " link="/" />
          <UiBaseButton :label="i18n.t('projects')" :type="isActive('projects') ? 'transparent-active' : 'transparent'"
            link="/projects" />
          <UiBaseButton :label="i18n.t('about')" :type="isActive('about') ? 'transparent-active' : 'transparent'"
            link="/about" />
          <LanguageSwitcher />
          <UiBaseIcon :icon="searchMode ? 'cross' : 'lookingGlass'" class="cursor-pointer" @click="jumpToSearch" />
        </div>
      </div>
        ...
        <div class="w-full flex flex-col items-center bg-background">
          <div class="w-full flex items-center mb-7 px-6 sm:px-12 lg:px-18">
            <div class="flex-1 flex justify-start items-baseline">
              ...
              <div v-if="onTagPage" class="text-title-m text-primary whitespace-nowrap">
            ...
            <div class="flex-1 flex justify-end items-center">
              <div v-if="onTagPage">
                <UiBaseBadge class="mr-10 cursor-pointer" @click="jumpToSearch">{{ i18n.t("searchDefault") }}
                </UiBaseBadge>
              </div>
              <UiBaseIcon icon="cross" class="cursor-pointer" @click="hideSearchResults = true" />
            </div>
          </div>
      ...
      <div v-if="searchMode" class="w-full flex flex-col items-center">
        <form @submit.prevent="search" class="w-full flex gap-10 items-center mb-8 px-6 sm:px-12 lg:px-18">
          <div class="text-title-m text-primary flex-999">
            <input v-model="searchQuery" ref="searchInput" type="text" :placeholder="i18n.t('searchDefault') + '...'"
              class="w-full focus:outline-none focus:border-primary focus:border-[1px]" />
          </div>
          <UiBaseBadge class="hidden lg:block"><router-link to="/tags">{{
            i18n.t("searchByTag")
              }}</router-link></UiBaseBadge>
          <UiBaseIcon icon="lookingGlass" class="cursor-pointer" @click="search" />
        </form>
        <UiBaseBadge class="block lg:hidden mb-6"><router-link to="/tags">{{
          i18n.t("searchByTag")
            }}</router-link></UiBaseBadge>
